...
public class BoardController {
    ...
    private String showAll(HttpSession session, Model model, @PathVariable int pageNo) {
        ...
        // 이것들을 이제 우리가 컨트롤 해주면 된다. ->시작,글 페이지 주석,코드 만듬
    ...
    @PostMapping("write")
    public String writer(HttpSession session, BoardDTO boardDTO) {
        UserDTO logIn = (UserDTO) session.getAttribute("logIn");
        if (logIn == null) {
            return "redirect:/";
        }
    ...
    @GetMapping("update/{id}")//사용자가 요청하는 링크 유아엘
    public String showUpdate(@PathVariable int id, HttpSession session, RedirectAttributes redirectAttributes, Model model) {
        ...
        if (boardDTO == null) {
            redirectAttributes.addFlashAttribute("message", "존재하지 않은 글 번호임");
            return "redirect:/showMessage";
        }

        if (boardDTO.getWriterId() != logIn.getId()) {
            redirectAttributes.addFlashAttribute("message", "권한이 없음");
            return "redirect:/showMessage";
        }
